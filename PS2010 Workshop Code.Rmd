...
mydata_long <- mydata %>%               # creates a new object called mydata_long
  pivot_longer(cols = c(before, after), # use pivolt_longer() and select the column names.
...
redcow_t <- t.test(bpm ~ time, mydata_long, paired = TRUE)

print(redcow_t)

# we also want to find and report the effect size. for a t-test we can use Cohen's d.
mydata_long %>% 
  cohens_d(bpm ~ time, paired = TRUE)
```
...
```{r, eval = FALSE}
# import data
mydata2 <- read.csv("redcowchoc.csv")
